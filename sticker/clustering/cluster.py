...
FILTER="finance"
FILTER=FILTER.lower()
directory= str(os.path.realpath(__file__)).replace('/clustering/cluster.py','')
...
for i in range(n):
    for j in range(n):
        if j<=i:
            # print i 
            # print j
            MATRIX[i][j]=cosine_similarity(documents[i],documents[j])
...
NETWORK={"nodes":[{"name":variable,"group":None} for variable in data["Name"]],
        "links":[]
        }
    ...
    for j in range(n):
        if j<i:
            if percentileofscore(NEW_MATRIX[i],NEW_MATRIX[i][j])>0.4 and NEW_MATRIX[i][j]>1.001:# and len():
                NETWORK['links'].append({"source":i,"target":j,"value":math.floor(NEW_MATRIX[i][j])})
                print len(data)
